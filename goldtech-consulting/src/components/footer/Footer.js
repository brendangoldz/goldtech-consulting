...
const Footer = ({ variant = 'consulting' }) => {
  ...
  return (
  <footer className={`${isMarketing ? 'bg-marketing-primary' : 'bg-navy'} text-gray-300 py-12`}>
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="grid md:grid-cols-3 gap-6 lg:gap-12">
        {/* Company Info */}
        <div className="md:text-left">
          <div className="mb-4 flex justify-center md:justify-start">
            <Logo size="large" variant={variant} className="filter brightness-0 invert" />
          </div>
        ...
        <div className="text-center md:text-left">
          ...
          <ul className="space-y-2 text-sm">
            ...
            <li><a href="#projects" className={`${isMarketing ? 'hover:text-white' : 'hover:text-gold'} transition-colors`}>Projects</a></li>
            <li><a href="#contact" className={`${isMarketing ? 'hover:text-white' : 'hover:text-gold'} transition-colors`}>Contact</a></li>
          </ul>
        </div>
        ...
        <div className="text-center md:text-left">
          ...
          <div className="flex justify-center space-x-4 md:justify-start">
            ...
            {contact.socialLinks.github && (
              <a href={contact.socialLinks.github} target="_blank" rel="noopener noreferrer" className={`p-2 rounded-lg border ${isMarketing ? 'border-white/30 hover:border-white hover:bg-white/10' : 'border-gray-600 hover:border-gold hover:bg-gold/10'} transition-all duration-300`} aria-label="View GitHub profile">
                <FaGithub className={`text-gray-300 ${isMarketing ? 'hover:text-white' : 'hover:text-gold'} transition-colors`} />
              </a>
            )}
            {contact.socialLinks.upwork && (
              <a href={contact.socialLinks.upwork} target="_blank" rel="noopener noreferrer" className={`p-2 rounded-lg border ${isMarketing ? 'border-white/30 hover:border-white hover:bg-white/10' : 'border-gray-600 hover:border-gold hover:bg-gold/10'} transition-all duration-300`} aria-label="View Upwork profile">
                <svg className={`w-5 h-5 text-gray-300 ${isMarketing ? 'hover:text-white' : 'hover:text-gold'} transition-colors`} viewBox="0 0 56.7 56.7" fill="currentColor">
